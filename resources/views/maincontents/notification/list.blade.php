                  ...
                  <tbody class="table-border-bottom-0">
                     <?php if($rows){ $sl=1; foreach($rows as $row){?>
                        <tr>
                            ...
                            <td>
                              <?php if($row->is_send){?>
                                <a href="<?=url($controllerRoute . '/change-status-send/'.Helper::encoded($row->id))?>" class="btn btn-outline-success btn-sm" title="Send <?=$module['title']?>"><i class="fa fa-check"></i>&nbsp;Send</a>
                              <?php } else {?>
                                <a href="<?=url($controllerRoute . '/change-status-send/'.Helper::encoded($row->id))?>" class="btn btn-outline-warning btn-sm" title="Not Send <?=$module['title']?>"><i class="fa fa-times"></i>&nbsp;Not Send</a>
                              <?php }?>
                            </td>
                            <td>
